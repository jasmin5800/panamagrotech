<div class="content">
    <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <ol class="breadcrumb float-right">
                        ...
                        <li class="breadcrumb-item"><a href="<?php echo base_url('acount/index');?>"><?php echo $page_title; ?></a></li>
                    </ol>
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card-box">
                  <h4 class="header-title m-t-0 text-center m-b-30"><?php echo (($method=="edit")?"Update":"Add")?> acount</h4>
                  <form id="<?php echo (($method=="edit")?"$frm_id":"$frm_id");?>" class="form-horizontal" role="form" >
                      <div class="form-group row">
                          <label for="name" class="col-3 col-form-label">acount Name<span class="text-danger"> *</span></label>
                          <div class="col-9">
                              <input placeholder="acount Name" type="text" name="name" title="acount Name" required="" data-parsley-required-message="acount Name is Required" class="form-control" autocomplete="off" value="<?php echo (($method=="edit")?$result->name:"");  ?>">
                          </div>
                      </div>
                      ...
                      <div class="form-group row">
                          ...
                          <?php echo (($method=="edit")?'<input type="hidden" name="id" value="'.$result->id_acount.'">':""); ?>
                      </div>
                      <?php } ?>
...
<script type="text/javascript">
  ...
  var table = $('#datatable-buttons').DataTable({
          ...
          dom: 'Blfrtip',
          ajax: {
                 "url": "<?php echo base_url('acount/getLists/'); ?>",
                 "type": "POST",
             },
    ...
    $("#Add_frm").submit(function(event){
        ...
        $.ajax({
            url: "<?php echo base_url('acount/create')?>",
            type: "POST",
            data: data,              
     ...
     $("#Edit_frm").submit(function(event){
        ...
        var data=$(this).serialize();            
        $.ajax({
            url: "<?php echo base_url('acount/update')?>",
            type: "POST",
            data: data,              
            success: function(result){
                var result  = JSON.parse(result);
                if(result.status=="success"){
                    swal("success",result.msg,"success","#4fa7f3").then(function () { 
                     window.location="<?php echo base_url("acount");?>";            
                });
                }else{
                    swal("error",result.msg,"warning","#4fa7f3");
                }
   ...
   $('#datatable-buttons').on('click', '[data-id=delete]', function () {                        
        ...
        swal({
            ...
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('acount/delete/');?>"+id+"",
