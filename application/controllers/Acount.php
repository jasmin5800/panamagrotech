...
class  Acount extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/data/acount',$data);
	...
	public function create()
	{
		...
		if(isset($name) && !empty($name) && isset($city_id) && !empty($city_id) && isset($state_id) && !empty($state_id)){
			...
			$count=$this->General_model->has_duplicate($name,'acount','name');
			if($count>0){
				$data['status']="error";
				$data['msg']="acount Already Exist" ;	
			}else{
				...
				$detail=$this->db->insert('acount',$detail);
				$data['status']="success";
				$data['msg']="acount Added";	
	...
	public function getLists(){
		$this->General_model->auth_check();
		$table='acount';
		$order_column_array=array('id_acount', 'name','mobile','address','city_name','status','created_at');
		$search_order= array( 'name','mobile','address','city_name');
		$order_by_array= array('id_acount' => 'ASC');
        $data = $row = array();
        ...
        foreach($Master_Data as $m_data){
            ...
            $data[] = 	[$i,	        				
    					...
    					$m_data->address,
    					$m_data->city_name,
    					'<a href="'.base_url('acount/get_editfrm/').$m_data->id_acount.'"><button type="button" class="btn btn-custom waves-effect waves-light"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>&nbsp;<button type="button" class="btn btn-danger waves-effect waves-light" data-id="delete" data-value="'.$m_data->id_acount.'"><i class="fa fa-trash" aria-hidden="true"></i></button>'];
        }
        $output = array(
            "draw" => $_POST['draw'],
	...
	public function get_editfrm($id)
	{
        ...
        $data['page_title']="acount";
		$data['result']=$this->General_model->get_row('acount','id_acount',$id);
		$data["city"]=$this->General_model->get_data('city','status','*','1');
		$data["state"]=$this->General_model->get_data('state','status','*','1');
		$this->load->view('admin/controller/header');
		$this->load->view('admin/controller/sidebar');
    ...
    public function update()
    {
    	...
    	if(isset($name) && !empty($name) && isset($city_id) && !empty($city_id) &&  isset($state_id) && !empty($state_id) && isset($id) && !empty($id)){
    		...
    		$count=$this->General_model-> has_duplicate_query("select name from acount where name ='".$name."' and id_acount !='".$id."'");
    		if($count>0){
    			$data['status']="error";
				$data['msg']="acount Already Exist" ;
    		...
    		$this->General_model->update('acount',$detail,'id_acount',$id);
    		$data['status']="success";
    		$data['msg']="acount Updated";
    ...
    public function delete($id)
    {
    	...
    	if(isset($id) && !empty($id)){
    		$detail=$this->General_model->delete('acount','id_acount',$id);
    		$data['status']="success";
    		$data['msg']="acount Deleted";
    	}else{
    		$data['status']="error";
    		$data['msg']="Something is Worng";				
    	}
