...
class PackingModel extends CI_Model 
{
    ...
    {   
        ...
        $query = $this->db->get('packing as t1');
    ...
    {   
        ...
        $this->db->join('sub_process as t2', 't1.sb_id = t2.id_sub_process', 'left');
        $this->db->join('master_admin as t5', 't1.user_id = t5.id_master', 'left'); 
        $query = $this->db->limit($limit,$start)->order_by($col,$dir)->get('packing as t1');
        if($query->num_rows()>0)
        {
            return $query->result(); 
        }
    ...
    {   
        $query=$this->db->select('t1.*,t2.name as process_name ,t5.username as user_name')->from('packing as t1')
                ...
                ->group_start()
                        ...
                        ->or_like('t1.packing_value',$search)
    ...
    {
        $query=$this->db->select('challan_no')->from('packing')
                ->order_by('packing_id',"DESC")
                ->limit('1')
                ->get();
